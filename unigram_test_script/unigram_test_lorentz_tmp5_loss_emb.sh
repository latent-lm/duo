...
run_hrfm_direct() {
  local trainable="$1" sweep_name="$2" proposal_type="$3" proposal_exp_rate="$4" hyper_T="$5"
  python unigram/unigram_test2_tmp5.py -m \
    hydra.job.chdir=true \
    hydra.sweep.dir=outputs/unigram_test2 \
    "hydra.sweep.subdir=${sweep_name}_rs${seed}" \
    +mode=tnb \
...
echo "================ HRFM Direct (cross_entropy) x learnemb ================"
# NELBO is fixed (PP strat 0.1), so the loss-side lists are safe as Hydra lists.
run_direct "True" "hrfm_direct_learnemb_unif_new" "unif" "1.0" "${UNIF_HYPER_T}"
run_direct "True" "hrfm_direct_learnemb_strat_new" "stratified_exp" "1.0,0.1,0.2,0.3,0.8,0.5" "1000"
