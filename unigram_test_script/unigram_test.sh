...
python unigram/unigram_test2.py -m \
  hydra.job.chdir=true \
  hydra.sweep.dir=outputs/unigram_test2 \
  'hydra.sweep.subdir=unigram_test2_losses_sp${max_steps}_lr${lr}_pt${proposal_type}_per${proposal_exp_rate}_ht${hyper_T}_vs${vocab_size}_polar' \
  +folder=artifacts \
  +vocab_size=2 \
  +max_steps=20000 \
  +lr=1e-5 \
...
python unigram/unigram_test2_cart.py -m \
  hydra.job.chdir=true \
  hydra.sweep.dir=outputs/unigram_test2 \
  'hydra.sweep.subdir=unigram_test2_losses_sp${max_steps}_lr${lr}_pt${proposal_type}_per${proposal_exp_rate}_ht${hyper_T}_vs${vocab_size}_cart' \
  +folder=artifacts \
  +vocab_size=2 \
  +max_steps=20000 \
  +lr=1e-5 \
